...
namespace invoice.Core.Interfaces.Services
{
    ...
    {
        Task<GeneralResponse<IEnumerable<PageReadDTO>>> GetAllAsync(string storeId);
        Task<GeneralResponse<PageReadDTO>> GetByIdAsync(string id);
        Task<GeneralResponse<PageReadDTO>> GetByTitleAsync(string title, string storeId);
        Task<GeneralResponse<IEnumerable<PageReadDTO>>> SearchAsync(string keyword, string storeId);
