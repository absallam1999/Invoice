...
namespace invoice.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        {
            return await _paymentRepo.CountAsync(userId, p => userId == null || p.UserId == userId);
        }
        ...
        public async Task<int> CountByInvoiceAsync(string invoiceId, string userId)
        {
            ...
            return await _paymentRepo.CountAsync(userId, p => p.InvoiceId == invoiceId && (userId == null || p.UserId == userId));
        }

        public async Task<int> CountByPaymentMethodAsync(string paymentMethodId, string userId)
        {
            if (string.IsNullOrWhiteSpace(paymentMethodId))
                return 0;

            return await _paymentRepo.CountAsync(userId, p => p.PaymentMethodId == paymentMethodId && (userId == null || p.UserId == userId));
        }
