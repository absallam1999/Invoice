...
namespace invoice.Services
{
    public class StoreService : IStoreService
    {
        ...
        public async Task<GeneralResponse<StoreReadDTO>> CreateAsync(StoreCreateDTO dto, string userId)
        {
            ...
            var user = await _useRepo.GetByIdAsync(userId);
            if (user == null)
                return new GeneralResponse<StoreReadDTO>(false, "User not found.");

            var storeDB = await _storeRepo.GetSingleByUserIdAsync(userId);
            if (storeDB != null)
                return new GeneralResponse<StoreReadDTO>(false, "You already own a store.");

            var exists = await _storeRepo.GetBySlugAsync(dto.Slug.Trim().ToLower());
            if (exists != null)
                return new GeneralResponse<StoreReadDTO>(false, "Slug already exists, please choose another name.");
